...
class BroadcastService(override val http: HttpRequestBuilder) extends BroadcasterCommon with TelegramRequestBuilder {
  ...
  def broadcastWithPhoto(fileInfo: FileInfo, byteSource: Source[ByteString, Any], formFields: Map[String, String])(implicit mat: Materializer, secretKey: String) : Future[BroadcastOutput] = {
    secretKey match {
      case SECRET_KEY =>
