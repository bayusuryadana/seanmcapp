...
class InstagramFetcherSpec extends AsyncWordSpec with Matchers {
  ...
  when(http.sendRequest(ArgumentMatchers.eq(initUrl), any())).thenReturn(initResponse)
  ...
  when(http.sendRequest(ArgumentMatchers.eq(fetchUrl), any())).thenReturn(fetchResponse)
